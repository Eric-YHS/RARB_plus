...
def main(args):
    ...
    reranker = None
    rerank_featurizer = None
    reranker_ckpt = getattr(args, "reranker_ckpt", None)
    if reranker_ckpt:
        ckpt = torch.load(reranker_ckpt, map_location="cpu")
        feat_cfg = ckpt["featurizer"]
        rerank_featurizer = RerankFeaturizer(
        ...
        reranker = MLPReactionReranker(
            input_dim=rerank_featurizer.feature_dim,
            hidden_dim=int(model_cfg.get("hidden_dim", 128)),
            dropout=float(model_cfg.get("dropout", 0.1)),
        )
        reranker.load_state_dict(ckpt["state_dict"], strict=True)
        reranker = reranker.to(torch_device).eval()
    ...
    for i, data in enumerate(tqdm(dataloader)):
        ...
        for true_mol, product_mol, pred_mols, pred_scores, nlls, ells in zip(
            ...
            for pred_mol, pred_score, nll, ell in zip(pred_mols, pred_scores, nlls, ells):
                ...
                if reranker is not None and rerank_featurizer is not None:
                    feat = rerank_featurizer(
                        product_smi,
                        pred_smi,
                        row={"nll": float(nll), "ell": float(ell)},
                    )
                    if feat is None:
                        rerank_logits.append(float("-inf"))
                        rerank_scores.append(0.0)
                    else:
                        x = torch.tensor(feat, dtype=torch.float32, device=torch_device).unsqueeze(0)
                        with torch.no_grad():
                            logit = float(reranker(x).squeeze(0).detach().cpu().item())
                        rerank_logits.append(logit)
                        rerank_scores.append(float(torch.sigmoid(torch.tensor(logit)).item()))
                else:
                    rerank_logits.append(np.nan)
                    rerank_scores.append(np.nan)
...
if __name__ == '__main__':
    ...
    parser.add_argument('--reranker_ckpt', action='store', type=str, required=False, default=None)
