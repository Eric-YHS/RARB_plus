...
class AugmentedGraphFeatureEncoder(nn.Module):
    ...
    def __init__(
        ...
        self.token_meta_dim = int(token_meta_dim)
        ...
        self.PE_layer = torch.nn.Linear(self.token_meta_dim, graph_emb_dim)
    ...
    def forward(self, y: Tensor):
        ...
        if y.size(1) <= self.raw_y_dim:
            return self.attn_act(graph_level_part)

        # assume the graph embedding is graph_emb_dim-dim and is followed by token_meta_dim scalars
        # (e.g., rank, similarity).
        reshape_emb = torch.reshape(
            y[:, self.raw_y_dim :], [y.size(0), -1, self.graph_emb_dim + self.token_meta_dim]
        )
        meta = reshape_emb[..., self.graph_emb_dim :]  # bs, k, meta_dim
        aug_graph_tokens = reshape_emb[..., : self.graph_emb_dim]  # bs, k, 512
        meta_PE = self.PE_layer(meta)  # bs, k, 512
        aug_graph_tokens = self.dropout0(aug_graph_tokens + meta_PE)
...
class GraphTransformer(nn.Module):
    ...
    def __init__(self, n_layers: int, input_dims: dict, hidden_mlp_dims: dict, hidden_dims: dict,
                 ...
                 retrieval_k=0, augmented_graphfeature=False, retrieval_token_meta_dim: int = 1, rxn_class_num: int = 0):
        ...
        if self.rxn_class_num > 0:
            self.rxn_class_head = nn.Sequential(
                nn.Linear(hidden_dims['dy'], hidden_mlp_dims['y']),
                act_fn_out,
                nn.Dropout(dropout),
                nn.Linear(hidden_mlp_dims['y'], self.rxn_class_num),
            )
        else:
            self.rxn_class_head = None

    def forward(self, X, E, y, node_mask, return_class_logits: bool = False):
        ...
        class_logits = self.rxn_class_head(y) if self.rxn_class_head is not None else None
