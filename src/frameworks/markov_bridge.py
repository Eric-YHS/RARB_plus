...
class MarkovBridge(pl.LightningModule):
    def __init__(
            ...
            retrieval_token_meta_dim: int = 1,
            rxn_class_num: int = 0,
            rxn_class_loss_weight: float = 0.0,
            rxn_class_offset: int = 0,
    ):
        ...
        self.retrieval_token_meta_dim = int(retrieval_token_meta_dim)

        self.rxn_class_num = int(rxn_class_num)
        self.rxn_class_loss_weight = float(rxn_class_loss_weight)
        self.rxn_class_offset = int(rxn_class_offset)
        ...
        self.model = GraphTransformer(
            ...
            retrieval_token_meta_dim=self.retrieval_token_meta_dim,
            rxn_class_num=self.rxn_class_num,
        )
    ...
    def _build_retrieval_emb(self, data, *, extra_retri: int = 2, random_select: bool = True):
        if self.retrieval_k <= 0:
            return None
        if self.encoded_reactants is None:
            raise ValueError("retrieval_k > 0 but encoded_reactants is None.")
        if not isinstance(self.encoded_reactants, torch.Tensor):
            raise TypeError("encoded_reactants must be a torch.Tensor when retrieval_k > 0.")
        if not hasattr(data, "retrieval_list"):
            raise ValueError("retrieval_k > 0 but batch data is missing `retrieval_list`.")

        retrieval_list = data.retrieval_list  # (bs, topK)
        retrieval_index = retrieval_list[..., : self.retrieval_k + extra_retri]  # (bs, k+extra)

        if self.encoded_reactants.device.type == "cpu":
            retrieval_emb = self.encoded_reactants[retrieval_index.detach().to("cpu")].to(self.device)
        else:
            retrieval_emb = self.encoded_reactants[retrieval_index.to(self.encoded_reactants.device)].to(self.device)

        rank_list = (
            torch.arange(1, self.retrieval_k + 1 + extra_retri, device=self.device)
            .unsqueeze(0)
            .unsqueeze(-1)
            .repeat(retrieval_index.size(0), 1, 1)
        )  # (bs, k+extra, 1)

        meta_parts = [rank_list]
        if self.retrieval_token_meta_dim >= 2:
            if hasattr(data, "retrieval_sim"):
                retrieval_sim = data.retrieval_sim[..., : self.retrieval_k + extra_retri].to(self.device).float()
            else:
                retrieval_sim = torch.zeros_like(retrieval_index, device=self.device, dtype=torch.float)
            meta_parts.append(retrieval_sim.unsqueeze(-1))  # (bs, k+extra, 1)

        meta = torch.cat(meta_parts, dim=-1)  # (bs, k+extra, meta_dim)
        retrieval_tokens = torch.cat([retrieval_emb, meta], dim=-1)  # (bs, k+extra, 512+meta_dim)

        if random_select:
            random_indices = torch.stack(
                [
                    torch.randperm(self.retrieval_k + extra_retri, device=self.device)[: self.retrieval_k]
                    for _ in range(retrieval_tokens.size(0))
                ],
                dim=0,
            )  # (bs, k)
            random_indices_expanded = random_indices.unsqueeze(-1).expand(
                retrieval_tokens.size(0), self.retrieval_k, retrieval_tokens.size(-1)
            )
            retrieval_tokens = retrieval_tokens.gather(1, random_indices_expanded)
        else:
            retrieval_tokens = retrieval_tokens[:, : self.retrieval_k, :]

        return retrieval_tokens.flatten(start_dim=1)  # (bs, k*(512+meta_dim))

    def _maybe_add_rxn_class_loss(self, out: dict, class_logits, data, batch_idx: int, stage: str):
        if self.rxn_class_num <= 0 or self.rxn_class_loss_weight <= 0:
            return out
        if class_logits is None:
            return out
        if not hasattr(data, "rxn_class"):
            return out

        labels = data.rxn_class.view(-1).to(class_logits.device).long() - int(self.rxn_class_offset)
        valid = (labels >= 0) & (labels < self.rxn_class_num)
        if valid.sum().item() == 0:
            return out

        class_loss = F.cross_entropy(class_logits[valid], labels[valid], reduction="mean")
        class_acc = (class_logits[valid].argmax(dim=-1) == labels[valid]).float().mean()

        out["loss"] = out["loss"] + self.rxn_class_loss_weight * class_loss

        if batch_idx % self.log_every_steps == 0:
            self.log(f"{stage}_loss/rxn_class", class_loss.detach())
            self.log(f"{stage}_acc/rxn_class", class_acc.detach())
    ...
    def process_and_forward(self, data):
        ...
        #(bs,k)
        retrieval_emb = self._build_retrieval_emb(data, extra_retri=2, random_select=True)
        ...
        pred_out = self.forward(noisy_data, extra_data, node_mask, return_class_logits=(self.rxn_class_num > 0))
        if isinstance(pred_out, tuple):
            pred, class_logits = pred_out
        else:
            pred, class_logits = pred_out, None
        ...
        return reactants, product, pred, node_mask, noisy_data, context, class_logits

    def training_step(self, data, i):
        reactants, product, pred, node_mask, noisy_data, _, class_logits = self.process_and_forward(data)
        if self.loss_type == 'vlb':
            out = self.compute_training_VLB(
                reactants=reactants,
                pred=pred,
                node_mask=node_mask,
            ...
            out = self.my_compute_training_VLB(
                reactants=reactants,
                pred=pred,
                node_mask=node_mask,
                noisy_data=noisy_data,
            ...
            out = self.compute_training_CE_loss_and_metrics(reactants=reactants, pred=pred, i=i)

        return self._maybe_add_rxn_class_loss(out, class_logits, data, batch_idx=i, stage="train")
    ...
    def validation_step(self, data, i):
        reactants, product, pred, node_mask, noisy_data, context, class_logits = self.process_and_forward(data)
        if self.loss_type == 'vlb':
            out = self.compute_validation_VLB(
                reactants=reactants,
                pred=pred,
                node_mask=node_mask,
                noisy_data=noisy_data,
            ...
            out = self.my_compute_validation_VLB(
                reactants=reactants,
                pred=pred,
                node_mask=node_mask,
                noisy_data=noisy_data,
            ...
            out = self.compute_validation_CE_loss(reactants=reactants, pred=pred, i=i)

        return self._maybe_add_rxn_class_loss(out, class_logits, data, batch_idx=i, stage="val")
    ...
    def forward(self, noisy_data, extra_data, node_mask, return_class_logits: bool = False):
        X = torch.cat((noisy_data['X_t'], extra_data.X), dim=2).float()
        E = torch.cat((noisy_data['E_t'], extra_data.E), dim=3).float()
        y = torch.hstack((noisy_data['y_t'], extra_data.y)).float()
        return self.model(X, E, y, node_mask, return_class_logits=return_class_logits)
