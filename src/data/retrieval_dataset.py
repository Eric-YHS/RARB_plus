...
class RetroBridgeDataset(InMemoryDataset):
    ...
    def process(self):
        ...
        for i, row in tqdm(table.iterrows(), total=table.shape[0]):
            retrieval_index_str = row.get('retrieval_index', '')
            retrieval_index_str = '' if pd.isna(retrieval_index_str) else str(retrieval_index_str)
            retrieval_index = [int(x) for x in retrieval_index_str.split(',') if str(x).strip() != '']
            retrieval_list = torch.tensor(retrieval_index, dtype=torch.long).unsqueeze(0)

            retrieval_sim_str = row.get('retrieval_similarity', '')
            retrieval_sim_str = '' if pd.isna(retrieval_sim_str) else str(retrieval_sim_str)
            if retrieval_sim_str.strip() == '':
                retrieval_sim = torch.zeros((1, len(retrieval_index)), dtype=torch.float)
            else:
                sims = [float(x) for x in retrieval_sim_str.split(',') if str(x).strip() != '']
                if len(sims) != len(retrieval_index):
                    # Be robust to mismatched lengths: pad/truncate to match indices.
                    sims = (sims + [0.0] * len(retrieval_index))[: len(retrieval_index)]
                retrieval_sim = torch.tensor(sims, dtype=torch.float).unsqueeze(0)
            ...
            rxn_class_val = row.get('class', -1)
            if pd.isna(rxn_class_val):
                rxn_class_val = -1
            try:
                rxn_class_val = int(rxn_class_val)
            except Exception:
                rxn_class_val = -1
            rxn_class = torch.tensor([rxn_class_val], dtype=torch.long)
            data = Data(
...
class RetroBridgeDatasetInfos:
    ...
    def compute_input_output_dims(
        self, datamodule, extra_features, domain_features, use_context, retrieval_k=0, retrieval_token_meta_dim: int = 1
    ):
        ...
        if not self.augmented_graphfeature:
            token_meta_dim = int(retrieval_token_meta_dim)
            if token_meta_dim < 1:
                raise ValueError("retrieval_token_meta_dim must be >= 1 (rank PE is always required).")
            self.input_dims['y'] += (512 + token_meta_dim) * retrieval_k
